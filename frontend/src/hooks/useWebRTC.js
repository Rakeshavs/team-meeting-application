...
export function useWebRTC(roomId, options = {}) {
    ...
    switch (type) {
      ...
      case 'caption':
        setParticipantsMetadata((prev) => ({
          ...
          [peerId]: {
            ...prev[peerId],
            name: data.senderName || prev[peerId]?.name || 'Participant',
            isSpeaking: true,
        ...
        setTimeout(() => {
          setParticipantsMetadata((prev) => {
            const metadata = prev[peerId];
            if (!metadata || Date.now() - (metadata.lastCaptionTime || 0) < 4500) {
              return prev;
            }
  ...
  const sendCaptionMessage = useCallback((text) => {
    sendSignalingMessage({
      type: 'caption',
      text,
